...
export const SystemSettings: React.FC = () => {
  ...
  // General Settings
  const { data: generalData, isLoading: generalLoading, refetch: refetchGeneral } = useCustom({
    url: "get-general-settings",
    method: "get",
  });

  const { mutate: createGeneralSetting, isLoading: creatingGeneral } = useCustomMutation();
  const { mutate: updateGeneralSetting, isLoading: updatingGeneral } = useCustomMutation();
  const { mutate: activateGeneralSetting, isLoading: activatingGeneral } = useCustomMutation();

  // Pricing Settings
  const { data: pricingData, isLoading: pricingLoading, refetch: refetchPricing } = useCustom({
    url: "get-pricing-settings",
    method: "get",
  });

  const { mutate: createPricingSetting, isLoading: creatingPricing } = useCustomMutation();
  const { mutate: updatePricingSetting, isLoading: updatingPricing } = useCustomMutation();
  const { mutate: activatePricingSetting, isLoading: activatingPricing } = useCustomMutation();

  // Payment Settings
  const { data: paymentData, isLoading: paymentLoading, refetch: refetchPayment } = useCustom({
    url: "get-payment-settings",
    method: "get",
  });

  const { mutate: createPaymentSetting, isLoading: creatingPayment } = useCustomMutation();
  const { mutate: updatePaymentSetting, isLoading: updatingPayment } = useCustomMutation();
  const { mutate: activatePaymentSetting, isLoading: activatingPayment } = useCustomMutation();

  // Security Settings
  const { data: securityData, isLoading: securityLoading, refetch: refetchSecurity } = useCustom({
    url: "get-security-settings",
    method: "get",
  });

  const { mutate: createSecuritySetting, isLoading: creatingSecurity } = useCustomMutation();
  const { mutate: updateSecuritySetting, isLoading: updatingSecurity } = useCustomMutation();
  const { mutate: activateSecuritySetting, isLoading: activatingSecurity } = useCustomMutation();

  // System Health
  const { data: systemHealthData, isLoading: healthLoading, refetch: refetchHealth } = useCustom({
    url: "get-system-health",
    method: "get",
  });

  const { data: systemHealthStats, isLoading: statsLoading, refetch: refetchStats } = useCustom({
    url: "get-system-health-stats",
    method: "get",
  });

  const { mutate: refreshSystemHealth, isLoading: refreshingHealth } = useCustomMutation();

  // Enhanced mutation handlers with automatic refetch
  const handleCreateSettings = async (section: string, values: any) => {
    const mutations = {
      general: () => createGeneralSetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to create general settings: ${error.message}`);
        },
      }),
      pricing: () => createPricingSetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to create pricing settings: ${error.message}`);
        },
      }),
      payment: () => createPaymentSetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to create payment settings: ${error.message}`);
        },
      }),
      security: () => createSecuritySetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to create security settings: ${error.message}`);
        },
      }),
    };
  ...
  const handleUpdateSettings = async (section: string, id: string, values: any) => {
    const mutations = {
      general: () => updateGeneralSetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to update general settings: ${error.message}`);
        },
      }),
      pricing: () => updatePricingSetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to update pricing settings: ${error.message}`);
        },
      }),
      payment: () => updatePaymentSetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to update payment settings: ${error.message}`);
        },
      }),
      security: () => updateSecuritySetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to update security settings: ${error.message}`);
        },
      }),
    };
  ...
  const handleSaveSettings = async (values: SettingsFormData, section: string) => {
    setLoading(true);
    try {
      const settingsData = {
  ...
  const handleActivateSettings = async (section: string, id: string) => {
    const mutations = {
      general: () => activateGeneralSetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to activate general settings: ${error.message}`);
        },
      }),
      pricing: () => activatePricingSetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to activate pricing settings: ${error.message}`);
        },
      }),
      payment: () => activatePaymentSetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to activate payment settings: ${error.message}`);
        },
      }),
      security: () => activateSecuritySetting({
        ...
        onError: (error: any) => {
          message.error(`Failed to activate security settings: ${error.message}`);
        },
      }),
    };
    ...
    <Spin spinning={generalLoading}>
        ...
        extra={
          generalData?.data?.id && (
            <Button
              ...
              onClick={() => handleActivateSettings('general', generalData.data.id)}
      ...
      >
        <Form
          ...
          initialValues={generalData?.data || {
    ...
    <Spin spinning={pricingLoading}>
        ...
        extra={
          pricingData?.data?.id && (
            <Button
              type="primary"
              icon={<CheckCircleOutlined />}
              onClick={() => handleActivateSettings('pricing', pricingData.data.id)}
      ...
      >
        ...
        <Form
          ...
          initialValues={pricingData?.data || {
            baseFare: 500,
            perKmRate: 150,
            perMinuteRate: 50,
            minimumFare: 800,
          ...
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Form.Item
                ...
                rules={[{ required: true, message: 'Per kilometer rate is required' }]}
              >
                <InputNumber 
                  min={0} 
                  style={{ width: '100%' }}
            ...
            <Col span={8}>
              <Form.Item
                ...
                rules={[{ required: true, message: 'Per minute rate is required' }]}
              >
                <InputNumber 
                  min={0} 
                  style={{ width: '100%' }}
          ...
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Form.Item
                name="maximumFare"
                label="Maximum Fare (₦)"
                rules={[{ required: true, message: 'Maximum fare is required' }]}
              >
                <InputNumber 
                  min={0} 
                  style={{ width: '100%' }}
            ...
            <Col span={8}>
              <Form.Item
                ...
                rules={[{ required: true, message: 'Surge multiplier is required' }]}
              >
                <InputNumber
                  min={1}
                  max={5}
    ...
    <Spin spinning={paymentLoading}>
        ...
        extra={
          paymentData?.data?.id && (
            <Button
              type="primary"
              icon={<CheckCircleOutlined />}
              onClick={() => handleActivateSettings('payment', paymentData.data.id)}
      ...
      >
        <Form
          ...
          initialValues={paymentData?.data || {
          ...
          <Row gutter={16}>
            <Col span={8}>
              <Form.Item
                ...
                rules={[{ required: true, message: 'Minimum wallet balance is required' }]}
              >
                <InputNumber 
                  min={0} 
                  style={{ width: '100%' }}
            ...
            <Col span={8}>
              <Form.Item
                ...
                rules={[{ required: true, message: 'Processing fee rate is required' }]}
              >
                <InputNumber
                  min={0}
                  max={10}
                ...
                <Row gutter={16}>
                  <Col span={12}>
                    <Form.Item
                      ...
                      rules={[{ required: true, message: 'Auto top-up threshold is required' }]}
                    >
                      <InputNumber 
                        min={0} 
                        style={{ width: '100%' }}
                  ...
                  <Col span={12}>
                    <Form.Item
                      ...
                      rules={[{ required: true, message: 'Auto top-up amount is required' }]}
                    >
                      <InputNumber 
                        min={0} 
                        style={{ width: '100%' }}
    ...
    <Spin spinning={securityLoading}>
        ...
        extra={
          securityData?.data?.id && (
            <Button
              type="primary"
              icon={<CheckCircleOutlined />}
              onClick={() => handleActivateSettings('security', securityData.data.id)}
      ...
      >
        <Form
          ...
          initialValues={securityData?.data || {
        ...
        >
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item
                ...
                rules={[{ required: true, message: 'Minimum password length is required' }]}
              >
                <InputNumber 
                  min={6} 
                  max={32} 
          ...
          <Form.Item
            noStyle
            shouldUpdate={(prevValues, currentValues) => 
              prevValues.enableIpWhitelisting !== currentValues.enableIpWhitelisting
            }
          >
            {({ getFieldValue }) =>
              getFieldValue('enableIpWhitelisting') && (
                <Form.Item
                  ...
                  rules={[{ required: true, message: 'At least one IP address is required' }]}
                >
                  <Select mode="tags" placeholder="Enter IP addresses (e.g., 192.168.1.1)" />
                </Form.Item>
              )
            }
          </Form.Item>
              ...
              <Card size="small" title="Database">
                <Space direction="vertical" style={{ width: '100%' }}>
                  ...
                  <Progress 
                    ...
                    status={
                      (systemHealthData?.data?.databaseUsagePercent || 0) > 90 ? 'exception' : 
                      (systemHealthData?.data?.databaseUsagePercent || 0) > 75 ? 'warning' : 'active'
                    }
                  />
                  <Text type="secondary">
              ...
              <Card size="small" title="Storage">
                <Space direction="vertical" style={{ width: '100%' }}>
                  ...
                  <Progress 
                    ...
                    status={
                      (systemHealthData?.data?.storageUsagePercent || 0) > 90 ? 'exception' : 
                      (systemHealthData?.data?.storageUsagePercent || 0) > 75 ? 'warning' : 'active'
                    }
                  />
                  <Text type="secondary">
            ...
            <Col span={6}>
              <Card size="small" title="Users">
                <Space direction="vertical" style={{ width: '100%' }}>
                  ...
                  />
                  <Progress 
                    percent={systemHealthData?.data?.userUtilizationPercent || 0}
                    status="active"
                  />
                  <Text type="secondary">
            ...
            <Descriptions bordered column={2}>
              ...
              <Descriptions.Item label="Last Backup">
                {systemHealthStats.data.system?.lastBackup ? 
                  new Date(systemHealthStats.data.system.lastBackup).toLocaleString() : 
                  'N/A'
                }
              </Descriptions.Item>
            </Descriptions>
